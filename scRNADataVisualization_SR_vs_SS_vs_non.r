...
# ============== Differential Expressed gene list for Fig. S5d =====================
...
# ===================== Marker Genes for Metascape Pathway Enrichment (Fig. S4f) =====================
...
# ====================================================================================
# Panel 2: STAT1_TARGETS vs. T_cell_activation Fig. S7c upper panel
# ====================================================================================
plot_correlation(
...
# ====================================================================================
# Panel 3: STAT1_TARGETS vs. GR_PATHWAY  Fig. S7c lower panel
# ====================================================================================
plot_correlation(
  x = data$STAT1_TARGETS,
  y = data$GR_PATHWAY,
...
##=============Fig. S6a Heatmap of GC-related Pathway Module Scores (Z-scored) ========================##
...
# == Boxplot_plot of selected genes across non, SS, and SR groups == #
# ===========================  Fig. S5e ============================ #
...
# Load expression data for plotting
data <- FetchData(CD8TEM, vars = c("novel_type", "IL2RB", "IL27RA", "CXCR6", "TNFRSF9", "CD27"))

# Define custom colors for group fill (保持分组颜色)
colors <- c("#337AB7", "#F0AD4E", "#D9534F")  # non, SS, SR

# Set group order for proper plotting
data$novel_type <- factor(data$novel_type, levels = c("non", "SS", "SR"))

# Define a list of target genes
genes_of_interest <- c("IL2RB", "IL27RA", "CXCR6", "TNFRSF9", "CD27")

# Function to create a single Boxplot + Jitter Plot for each gene (only mean line)
plot_gene <- function(gene_name) {
  p <- ggplot(data, aes(x = novel_type, y = .data[[gene_name]], fill = novel_type)) +
    geom_boxplot(width = 0.4, outlier.shape = NA, color = "black") +
    geom_jitter(
      position = position_jitter(width = 0.2, height = 0),
      size = 0.4, alpha = 0.3, color = "black", 
      show.legend = FALSE
    ) +
    scale_fill_manual(values = colors) +
    stat_summary(fun = mean, geom = "crossbar", 
                 width = 0.3, color = "black", size = 0.6) + # Only mean line (black)
...
plots <- lapply(genes_of_interest, plot_gene)

# Arrange in a 2x3 grid, with the last plot centered in the last row
combined_plot <- ggarrange(
  plots[[1]], plots[[2]], plots[[3]],
  plots[[4]], plots[[5]], NULL,  # Leaving the last position empty
...
ggsave(
  filename = "Combined_Boxplot_Mean_Line_Small_Jitter.pdf",
  plot = combined_plot,
  device = "pdf",
  width = 12,
  height = 8
...
# ==================Fig. S4a Expression analysis of RPS4Y1 across samples ================== #
...
ggsave("cloneType_distribution.pdf", plot = p13, width = 7, height = 5)
...
pdf("clonalOverlay_plot.pdf", width = 7, height = 6)
print(p11)
dev.off()
...
# ===================== Fig. S4g: Interaction Heatmap ===================== #
# Plot interaction count heatmap for SR group
pdf("FigS4g_SR_InteractionCount_pheatmap1.pdf", width = 6, height = 5)
pheatmap::pheatmap(
...
# ===================== Multi-group Comparison and Fig. S4h ===================== #
...
pdf("FigS4h_LR_Diff_BubblePlot_CD8TEM.pdf", width = 10, height = 5)
(gg1 | gg2) + 
